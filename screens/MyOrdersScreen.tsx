...
export default function MyOrdersScreen() {
  ...
  const handleUpdateBookingStatus = async (bookingId: string, status: 'completed' | 'cancelled' | 'pending' | 'confirmed') => {
    try {
        ...
        <View className="flex-row">
          ...
          <View className="flex-1">
            <View className="flex-row justify-between items-start mb-2">
              ...
              <View className={`px-2 py-1 rounded-full bg-success-500`}>
                <Text className="text-white font-medium text-xs">
                  Tersedia
                </Text>
              </View>
            ...
            <View className="flex-row items-center mb-2">
              ...
              <Text className="text-gray-500 text-sm ml-1">
                {format(new Date(food.pickup_time_end), 'dd MMM, HH:mm', { locale: id })}
              </Text>
            </View>
  ...
  const renderContent = () => {
    ...
    if (activeTab === 'donations') {
      ...
      return (
        ...
        >
          {myDonations.map((food: Food) => renderDonationCard(food as FoodWithBookings))}
        </ScrollView>
      )
    } else {
